...
export const createApp = (): Application => {
  ...
  // ── GraphQL ───────────────────────────────────────────────────────────────
  app.use('/api/v1/graphqlCategory', createHandler({ schema: categorySchema }));

  // ── REST API v1 ───────────────────────────────────────────────────────────
  app.use('/api/v1/auth', allRouters.authRouter);
  app.use('/api/v1/category', allRouters.categoryRouter);
  app.use('/api/v1/subCategory', allRouters.subCategoryRouter);
  app.use('/api/v1/brand', allRouters.brandRouter);
  app.use('/api/v1/product', allRouters.productRouter);
  app.use('/api/v1/coupon', allRouters.couponRouter);
  app.use('/api/v1/cart', allRouters.cartRouter);
  app.use('/api/v1/order', allRouters.orderRouter);
  app.use('/api/v1/review', allRouters.reviewRouter);

  // ── 404 fallback ─────────────────────────────────────────────────────────
  app.all('*', (req, _res, next) => next(new AppError(`Cannot find ${req.method} ${req.originalUrl}`, 404)));
