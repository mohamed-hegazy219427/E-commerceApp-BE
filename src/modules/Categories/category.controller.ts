...
export const updateCategory = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (name) {
    if (category.name === name.toLowerCase()) return next(new AppError(req.t.category.sameOldName, 400));
    if (await categoryModel.findOne({ name })) return next(new AppError(req.t.category.duplicateName, 409));
    category.name = name;
...
export const deleteCategory = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (category.subCategories.length) {
    ...
    if (!result.deletedCount) return next(new AppError(req.t.category.deleteSubCategoriesFailed, 500));
  }
  ...
  if (category.customId) {
    await cloudinary.api.delete_resources_by_prefix(`${env.PROJECT_FOLDER}/Categories/${category.customId}`);
